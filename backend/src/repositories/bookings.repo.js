...
async function saveRazorpayOrder(id,orderId){const{rows}=await query(`UPDATE bookings SET razorpay_order_id=$1,updated_at=now() WHERE id=$2 RETURNING *`,[orderId,id]);return toBooking(rows[0]);}
async function getRazorpayOrderId(id){const{rows}=await query(`SELECT razorpay_order_id FROM bookings WHERE id=$1`,[id]);return rows[0]?.razorpay_order_id||"";}
...
module.exports={findById,list,findOverlapping,slotsForDate,create,decide,cancel,statsForUser,upcoming,recent,markPaid,saveRazorpayOrder,getRazorpayOrderId,requestRefund,decideRefund,refundsQueue,serviceConflicts,ConflictError};
